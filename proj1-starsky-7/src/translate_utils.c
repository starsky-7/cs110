...
int translate_num(long int* output, const char* str, ImmType type) {
  ...
  char* endptr = NULL;
  long int result = strtol(str, &endptr, 0); // Correctly pass endptr
  if (endptr == str || *endptr != '\0') {
    return -1; // Invalid conversion, extra characters exist
  }

  // Validate the immediate value according to the given type
  if (is_valid_imm(result, type)) {
    *output = result;
    return 0;
  } 
  else {
    return -1; // Immediate value is out of valid range for the given type
  }
}
