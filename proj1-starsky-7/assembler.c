...
int pass_one(FILE* input, Block* blk, SymbolTable* table) {
  ...
  1. Skip all comments
     ...
     while (fgets(buf, BUF_SIZE, input)) {
      ...
      // the first token: maybe label or ins_name or invalid 
      ...
      int label_result = add_if_label(line_num, token, offset, table);
      ...
      /*the only difference between label_result == 0 and 1 
      is that when 1: the second token becomes ins_name */ 
      if (label_result == 1) {
        token = strtok(NULL, IGNORE_CHARS);// next token: name or nothing
      }
      if (token == NULL) { // there is nothing after label
        continue;
      }                 

      char* name = token;
      token = strtok(NULL, IGNORE_CHARS);
      // if the token after name is null, it means there is no arg, like ecall
      while (token != NULL && args_num < MAX_ARGS) { 
        args[args_num++] = token;
        token = strtok(NULL, IGNORE_CHARS);
      }

      /* larger than 3, invalid! skip */
      if (token != NULL) {
        errors++;
        raise_extra_argument_error(line_num, token);
        continue;
      } 

      // fixme：注意如果参数多于3个，这里的argsnum最多显示到3
      int inst_result = write_pass_one(blk, name, args, args_num);

      /* fail to write instru */
      if (inst_result == 0) {
        errors++;
        raise_instruction_error(line_num, token, args, args_num);
        continue;
      }
      offset += inst_result * 4;
...
int pass_two(Block* blk, SymbolTable* table, FILE* output) {
  ...
  for (uint32_t i = 0; i < blk->len; ++i) {
    ...
    line_num++;
    name = inst->name;
    for(uint32_t j = 0; j < inst->arg_num; j++){
      args[j] = inst->args[j];
    }
    args_num = inst->arg_num;
    if(translate_inst(output, name, args, args_num, offset, table) == -1){
      errors++;
      raise_instruction_error(line_num, name, args, args_num);
      continue;
    }
    offset += 4;
    /* === end === */
